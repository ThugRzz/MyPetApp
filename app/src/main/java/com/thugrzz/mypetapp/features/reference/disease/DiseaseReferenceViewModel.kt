...
class DiseaseReferenceViewModel(
    ...
    init {
        loadDiseaseReferences()
    }

    private fun loadDiseaseReferences() = viewModelScope.launch {
        innerIsLoadingFlow.emit(true)
        try {
            val references = networkRepository.getDiseaseReferences()
